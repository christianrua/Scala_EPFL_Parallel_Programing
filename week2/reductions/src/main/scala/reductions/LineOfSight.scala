...
object LineOfSight extends LineOfSightInterface {
  ...
  def max(x: Float, y: Float): Float = if (x > y) x else y

  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    var xAxis = 1
    while (xAxis < input.length) {
      val angle = input(xAxis) / xAxis
      val prevAngle = output(xAxis - 1)
      output(xAxis) = max(angle, prevAngle)
      xAxis += 1
    }
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    def auxUpsweepSequential(fromColx: Int, untilColx: Int, maxAngle: Float): Float = {
      if (fromColx == until) maxAngle
      else auxUpsweepSequential(fromColx + 1, untilColx, max(input(fromColx) / fromColx, maxAngle))
    }
    auxUpsweepSequential(from, until, 0f)
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    var xAxis = from + 1
    while (xAxis < until) {
      output(xAxis) = max(output(xAxis - 1), input(xAxis) / xAxis)
      xAxis += 1
    }
  }
