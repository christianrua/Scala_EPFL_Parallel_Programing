...
object ParallelCountChange extends ParallelCountChangeInterface {
  ...
  def parCountChange(money: Int, coins: List[Int], threshold: Threshold): Int = {
    ...
    //else if (money < coins.head) 0
      ...
      val (deductMoney, removeCoins) =
        parallel(parCountChange(money - coins.head, coins, threshold), parCountChange(money, coins.tail, threshold))
      deductMoney + removeCoins
    }
  }

  /** Threshold heuristic based on the starting money. */
  def moneyThreshold(startingMoney: Int): Threshold =
    (money, _) => money.toLong <= startingMoney * 2L / 3L

  /** Threshold heuristic based on the total number of initial coins. */
  def totalCoinsThreshold(totalCoins: Int): Threshold =
    (_, coins) => coins.size.toLong <= totalCoins * 2L / 3L


  /** Threshold heuristic based on the starting money and the initial list of coins. */
  def combinedThreshold(startingMoney: Int, allCoins: List[Int]): Threshold = {
    (money, coins) => money * coins.size <= startingMoney * allCoins.size / 2L
  }
}
