...
package object barneshut {
  ...
  extends Quad {
    ...
    def insert(b: Body): Quad = {
      if (size > minimumSize) {
        ...
        val position = size / 4
        val fork = Fork(
          Empty(centerX - position, centerY - position, sc),
          Empty(centerX + position, centerY - position, sc),
          Empty(centerX - position, centerY + position, sc),
          Empty(centerX + position, centerY + position, sc)
        )
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) extends SectorMatrixInterface {
    ...
    def +=(b: Body): SectorMatrix = {
      val axisX = math.min(math.max(boundaries.minX, b.x), boundaries.maxX)
      val axisY = math.min(math.max(boundaries.minY, b.y), boundaries.maxY)
      apply(((axisX - boundaries.minX) / sectorSize).toInt, ((axisY - boundaries.minY) / sectorSize).toInt) += b
      this
    }
