...
class KMeans extends KMeansInterface {
  ...
  def update(classified: Map[Point, Seq[Point]], oldMeans: Seq[Point]): Seq[Point] = {
    for {
      oldMean <- oldMeans
      point <- classified.get(oldMean)
    } yield findAverage(oldMean, point)
  }
  ...
  def converged(eta: Double, oldMeans: Seq[Point], newMeans: Seq[Point]): Boolean = {
    oldMeans.zip(newMeans).forall{case (prevMean, nextMean) => prevMean.squareDistance(nextMean) <= eta}
  }
  ...
  final def kMeans(points: Seq[Point], means: Seq[Point], eta: Double): Seq[Point] = {
    val classi = classify(points, means)
    val nextMeans = update(classi, means)
    if (!converged(eta,means, nextMeans)) {
      kMeans(points, nextMeans, eta)
    } else nextMeans
